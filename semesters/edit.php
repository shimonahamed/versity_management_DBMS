...
if (isset($_POST['submit'])) {
    ...
    if ($semester_name === '' || $semester_code === '' || $semester_number === '') {
        ...
        $status_val = $semester['status'];

        try {
            $stmt = mysqli_prepare(
                    $conn,
                    "UPDATE semester 
                 SET semester_name = ?, semester_code = ?, semester_number = ?, status = ? 
                 WHERE id = ?"
            );
                ...
                <div class="card-body p-4">
                    <form action="edit.php?id=<?= $id ?>" method="post">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="semester_code" value="<?= htmlspecialchars($semester['semester_code']) ?>" required>
                        </div>
                        <div class="mb-3">
